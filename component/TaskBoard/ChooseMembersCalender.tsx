...
export default function ChooseMembersCalender(props: IChooseMembersProps) {
    ...
    // Call the parent's setSelectedUsers when the selection changes
    React.useEffect(() => {

        props.setSelectedUsers(props.selectedUsers);
    }, [props.selectedUsers]);
                    ...
                    <View className={"flex flex-row space-x-6"}>
                        {props.users.map((user, index) => (
                            <View key={`${index}`}  className={"flex flex-col space-y-2"}>
                                ...
                                {props.selectedUser !==null && props.selectedUser.id== user.id  ?
                                   null
